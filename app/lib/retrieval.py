...
def _limit(a: dict, default: int = 10, cap: int = 34) -> int:
    try:
        return max(1, min(int(a.get("limit", default)), cap))
...
def channel2(con: sqlite3.Connection, codes: list[str], terms: list[str],
    ...
    allowed = set(by_code) if codes else None
    hits = _search("all", terms, allowed, k)
    for h in hits:
        cls = by_code.get(h["record_id"], [])
        h["_codes"] = [c["code"] for c in cls]
        spans = [c["evidence_span"] for c in cls if c.get("evidence_span")]
        h["_span"] = spans[0] if spans else None
...
def _satisfied(kind: str, codes: list[str], got: Retrieved) -> tuple[bool, str]:
    ...
    """
    if kind == "method":
        return (bool(got.method.get("flags")), "no method flags retrieved")

    if kind == "verbatim":
        n = len(got.verbatims)
        return (n >= 2, f"only {n} verbatim record(s) retrieved, need 2")

    if kind == "counter_evidence":
        c = got.counter
        n = len(c.get("rivals", [])) + len(c.get("complications", [])) + len(c.get("residual", []))
        return (n >= 1, "no disconfirming evidence retrieved")

    if kind == "external":
        return (bool(got.external), "no curated research matched")

    rows = _rows_for_kind(kind, got)
    if not rows:
        return (False, f"no {kind} rows retrieved")
    ...
    if counted:
        ...
        if not ok:
            ...
            return (False, f"{kind}: largest n is {best:g}, below the {floor}-record "
                           f"floor{' for a ranked claim' if kind == 'ranking' else ''}")
        if codes:
            missing = [c for c in codes
                       if not any(str(r.get("code", "")).upper() == c and
                                  (r.get("n") or 0) >= floor for r in counted)]
            if missing and len(missing) == len(codes):
                return (False, f"{kind}: nothing above the floor for "
                               f"{', '.join(missing)}")
            if missing:
                return (False, f"{kind}: below the {floor}-record floor for "
                               f"{', '.join(missing)}")
    return (True, "")
...
def gate(plan: dict, got: Retrieved) -> Verdict:
    ...
    if unmet:
        return Verdict("PARTIAL", met, unmet, reasons)
    return Verdict("FULL", met, unmet, [])
